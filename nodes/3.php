...
echo '<li>Managed chemicals for facilities across the Continental United States through the development of a chemical/product management system, which manages safety data sheets, governs chemical approvals, generates GHS compliant product labels, inventories chemicals, tracks materials, etc.</li>';
echo '<li>Trained, assessed, certified, and managed records for thousands of individuals by programming an online learning system that offers courses that meet training requirements defined by clients and federal, state, and local regulations.</li>';
echo '<li>Enabled clientele and field service personnel to obtain instantaneous facility reports by developing a comprehensive environmental reporting system, which generates air emission, chemical, groundwater, stormwater, et al. reports.</li>';
echo '<li>Protected clientele by introducing a compliance assurance system including the following: management of change, file management, calendar/events, resource center, et al.</li>';
echo '<li>Protected clientele content and trade secrets from unauthorized users by developing a secure authentication system that manages users, groups, and facilities permissions; provides encryption; access history; and brute force protection.</li>';
echo '<li>Architected the development of SaaS services through the design of databases, tables, relationships, and automated routines.</li>';
...
echo '<li>Reduced development costs and times by introducing Agile Development (including SCRUM) into company\'s pipeline.</li>';
...
echo '<li>Increased client retention by designing a mailing list system and working with vendors to create promotional artwork and items including: shirts, signs, lanyards, pens, mouse pads, flash lights, mats, holiday cards (2013 to present), mugs, flash drives, et al.</li>';
echo '<li>Improved SEO ranking and creditability by revamping the company\'s main website with in-house CMS, Bootstrap, et al. technologies.</li>';
...
echo '<li>Fought to get clientele back on track by authoring site-specific plans, reports, and systems designed to meet applicable regulations.</li>';
echo '<li>Designed facility sitemaps to accompany deliverables using CAD (DraftSight), SketchUp, Kerkythea (Render), ZBrush, etc.</li>';
...
echo '<ul>';
echo '<li>Programmed RJ-32 FANUC&reg; robotics systems to get manufacturing robots to meet manufacturer production requirements using VB.NET, .NET, FANUC&reg; TP (self-taught), KAREL (self-taught), HTML5, CSS3, JavaScript, et al.</li>';
echo '<li>Replaced inefficient multiple programs process with single precise system that ran every part flawlessly; increased quality and allowed for the reduction of metal material costs by 50%; reduced setup times by =600%; reduced downtimes (e.g. unnecessary and/or slow movements, time spent enabling protective shield, etc.); virtually eliminated scrap even on warped parts; maximized production rates; increased buff wheel life by 300%; reduced compound usage by >50%, which reduced cleaning efforts; reduced operator knowledge requirements; protected the safety of employees through safety parameter checks; accounted for environmental variables such as material selection, part specification, wheels\' dimensions, equipped jigs, et al.; etc.</li>';
echo '<li>Programmed GUI application using .NET that displayed robot\'s stats, allowed users to select input part settings (e.g. width, height, etc.), and featured rich web documentation to assist operators.</li>';
echo '<li>Programmed robots to perform square root calculations and other algorithms (such as Pythagorean Theorem, warp offset modifiers, etc.), which were used to determine operator\'s daily production rates, optimal target paths, and coordinates.</li>';
echo '<li>Collaborated with Maintenance and R&D to design new product lines and robot attachments.</li>';
echo '</ul>';
echo '</li>';
echo '</ul>';
echo '</li>';
...
echo '<li>Authored Game Design Document, which defined requirements of the development.</li>';
echo '<li>Programmed 2D isometric game engine for Portable Sony PlayStation&reg; primarily using Lua.</li>';
...
echo '<ul>';
echo '<li><i><b>Marlboro College Graduate &amp; Professional Studies</b> &mdash; Brattleboro, Vermont.</i><span class="right">2016</span>';
echo '<ul>';
echo '<li>Guest speaker for Web Design and Media Production class on October 9, 2016. Discussed custom coding Wordpress sites.</li>';
echo '</ul>';
echo '</li>';
echo '<li><i><b>JNH Environmental Services, Inc.</b> (Turnkey EHS Services Provider) &mdash; Rutland, Vermont.</i><span class="right">2012 to 2016</span>';
echo '<ul>';
echo '<li>Provided instruction to online students and updated classroom materials for courses including IATA - Dangerous Goods Regulations, Resource Conservation and Recovery Act, Department of Transportation Hazardous Materials Regulations, Globally Harmonized System, and Clean Water Act.</li>';
echo '</ul>';
echo '</li>';
echo '<li><i><b>Department of Motor Vehicles Agency of Transportation</b> (State Agency) &mdash; St. Johnsbury, Vermont.</i><span class="right">2004</span>';
echo '<ul>';
