...
echo "<p>The concept of luck is a perplexing problem to approach programmatically because of its intangibility. You are either lucky or you are not and your luck can change depending on the situation. You shouldn't be lucky all the time or unlucky all the time for that matter. Everyone's luck should be in a constant state of change. Luck should only be a contributing factor to help determine whether an action is a success and not necessarily the deciding factor. It should be elusive in value, its ability to change, and applicability.</p><p>Based on this vision of how luck works, I created the following roll class for use in my latest game engine, which is used to determine a character's chances of succeeding at an action and is influenced by luck. I originally programmed this in C#, but I ported the following portion of the class to PHP.</p>";
echo "<h3>Code</h3>";
?>
<pre>
/*
...
echo "<p>Tags: ";
echo "<span class=\"badge\">Web Design and Development</span>, <span class=\"badge\">Game Design</span>";
echo "</p>";

$results = $db->query("SELECT `post_id`,`title`,`content`,`date`,`last_modified` FROM `posts`");
...
foreach ($results as $row){
	...
	$db->bind("post_id",$row["post_id"]);
	$results2 = $db->query("SELECT `tags`.`title` FROM `post_tags` LEFT JOIN `tags` ON `post_tags`.`tag_id` = `tags`.`tag_id` WHERE `post_id` = :post_id;");
	echo "<p>Tags: ";
	foreach($results2 as $row2){
		echo "<span class=\"badge\">{$row2["title"]}</span>";
	}
	echo "</p>";
	echo "</div>";
}
