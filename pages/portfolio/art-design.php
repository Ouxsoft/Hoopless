...
if(count($results)>0){
	...
	foreach ($results as $row){
		...
		echo '<div class="thumbnail"><a href="'.portfolio_href('img', $row['href']).'" alt="'.$row['title'].'" data-toggle="lightbox" data-gallery="multiimages" data-title="'.$row["title"].'"><img src="'.portfolio_href('thumbnail', $row['thumbnail']).'" alt="Project"/></a></div>';
		echo '</div>';
	}
	echo '</div>';
}
